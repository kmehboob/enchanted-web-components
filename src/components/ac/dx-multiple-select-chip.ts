...
// Helper imports
import { DxMultiSelectInputFieldType, OptionData } from '../../types/dx-multiple-select-chip';
...
@localized()
export class DxMultipleSelectChip extends DxAcBaseElement {
  @state()
  ...
  @property({ type: String })
  field: DxMultiSelectInputFieldType | string = '';
  ...
  render() {
    ...
    > <div part="${INPUT_MULTI_SELECT_PARTS.MULTIPLE_SELECT_DIV_LABEL_AND_REMOVE}">
      ...
      <div
        ...
        @click=${(event: Event) => {
          // Close other open dx-multiple-select-chip dropdowns when this one is clicked.
          document.querySelectorAll('dx-multiple-select-chip').forEach((el) => {
...
declare global {
  interface HTMLElementTagNameMap {
    'dx-multiple-select-chip': DxMultipleSelectChip;
  }
}
